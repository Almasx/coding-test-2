...
type MetafieldValue = {
  title: string;
  discountType: "PERCENTAGE" | "FIXED_AMOUNT" | "FIXED_BUNDLE_AMOUNT";
  products: string[];
  collections: string[];
  tiers: Tier[];
};
...
function calculateBestDiscountCombo(
  ...
  const targets = Object.values(
    groupBy(expandedLineItems, (item) => item.id)
  ...
  const getDiscountValue = (): PercentageValue | FixedAmountValue => {
    if (options.discountType === "PERCENTAGE") {
      return { percentage: { value: appliedTier.amount.toString() } };
    }
    if (options.discountType === "FIXED_AMOUNT") {
      return { fixedAmount: { amount: appliedTier.amount.toString() } };
    }

    const originalTotalPrice = lineItems.reduce(
      (sum, item) =>
        sum + parseFloat(item.cost.amountPerQuantity.amount) * item.quantity,
      0
    );

    const discountAmount = Math.max(originalTotalPrice - appliedTier.amount, 0);

    return { fixedAmount: { amount: discountAmount.toString() } };
  };
